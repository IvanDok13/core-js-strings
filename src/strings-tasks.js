...
function countVowels(str) {
  const answ = str.match(/[aeiouy]/gi);
  return answ ? answ.length : 0;
}
...
function isPalindrome(str) {
  const removeChar = str.replace(/[\W_]/g, '');
  const repStr = removeChar.split('').reverse().join('');
  return repStr.toLowerCase() === removeChar.toLowerCase();
}
...
function reverseWords(str) {
  ...
  const newArr = [];
  arr.forEach((arg) => {
    newArr.push(arg.split('').reverse().join(''));
  });
  return newArr.join(' ');
}
...
function invertCase(str) {
  let answ = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toUpperCase()) answ += str[i].toLowerCase();
    else answ += str[i].toUpperCase();
  }
  return answ.trim();
}
...
function encodeToRot13(str) {
  ...
  let numOfChar = 0;
  for (let i = 0; i < str.length; i += 1) {
    numOfChar = str.charCodeAt(i);
    if (numOfChar > 64 && numOfChar < 78) {
      res += String.fromCharCode(numOfChar + 13);
    } else if (numOfChar > 77 && numOfChar < 91) {
      res += String.fromCharCode(numOfChar - 13);
    } else if (numOfChar > 96 && numOfChar < 110) {
      res += String.fromCharCode(numOfChar + 13);
    } else if (numOfChar > 109 && numOfChar < 123) {
      res += String.fromCharCode(numOfChar - 13);
    } else res += str[i];
  }
  return res.trim();
